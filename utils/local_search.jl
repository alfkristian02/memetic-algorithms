module LocalSearch
    export hamming_one_neighborhood_search

    include("../data/binary_decimal_conversion.jl")
    using .BinaryDecimalConversion: binary_to_decimal

    function get_neighborhood(individual::BitVector)::Vector{BitVector}
        n::Int = length(individual)
        neighbors::Vector{BitVector} = Vector{BitVector}(undef, n)
        for i in 1:n
    ...
    function hamming_one_neighborhood_search(children::Tuple{BitVector, BitVector}, all_fitnesses::Vector{Float64})::Tuple{BitVector, BitVector}
        best_children = Vector{BitVector}(undef, 2)

        for i in eachindex(children)
            candidates = get_neighborhood(children[i])
